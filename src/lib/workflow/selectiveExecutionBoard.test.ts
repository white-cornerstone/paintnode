...
describe('Workflow Board selective execution UX contract', () => {
  ...
  it('communicates provider-neutral candidate branch count, state, lineage, and retry', () => {
    ...
    expect(boardSource).toContain('Keyboard: F7 focuses candidates; F8 promotes the selected eligible candidate.');
    expect(boardSource).toContain('aria-keyshortcuts="F7 Alt+R"');
    expect(boardSource).toContain('Focus candidate review');
    expect(boardSource).toContain('aria-keyshortcuts="F8 Control+Enter"');
