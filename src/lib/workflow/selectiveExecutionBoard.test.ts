...
describe('Workflow Board selective execution UX contract', () => {
  ...
  it('communicates provider-neutral candidate branch count, state, lineage, and retry', () => {
    ...
    expect(boardSource).toContain('aria-keyshortcuts="P F8 Control+Enter"');
    ...
    expect(boardSource).toContain("event.key === 'F7'");
    expect(boardSource).toContain("event.key === 'F8'");
    expect(boardSource).toContain("event.key.toLowerCase() === 'r'");
    expect(boardSource).toContain("event.key.toLowerCase() === 'p'");
  ...
  it('registers every workflow AI entry point and locks its scoped nodes until terminal state', () => {
    expect(boardSource).toContain('Workflow: Generate ${candidateCount} concept branches');
    expect(boardSource).toContain("title: 'Workflow: Retry concept candidate'");
